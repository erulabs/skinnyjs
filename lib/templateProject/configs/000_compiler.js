  ...
  module.exports = function(app) {
    ...
    app.compiler['.coffee'] = function(file) {
      ...
      try {
        ...
        error = _error;
        return console.log(app.clr.cyan + 'CoffeeScript:' + app.clr.reset, 'not installed - try npm install "coffee-script"');
      }
      if (!coffee) {
    ...
    app.compiler['.scss'] = function(file) {
      var error;
      try {
        require.resolve('node-sass');
      } catch (_error) {
        error = _error;
        return console.log(app.clr.cyan + 'SASS:' + app.clr.reset, 'not installed - try npm install "node-sass"');
      }
      if (!sass) {
      ...
      return sass.render({
        ...
        error: (function(_this) {
          return function(error) {
            return console.log(app.clr.red + 'SCSS Compile error:' + app.clr.reset, error);
          };
        })(this)
      });
    };
    return app.compiler['.less'] = function(file) {
      var error;
      try {
        require.resolve('less');
      } catch (_error) {
        error = _error;
        return console.log(app.clr.cyan + 'LESS:' + app.clr.reset, 'not installed - try npm install "less"');
      }
      if (!less) {
        less = require('less');
      }
      ...
      return less.render(app.fs.readFileSync(file, 'utf8'), function(err, css) {
        return app.fs.writeFile(file.replace('.less', '.css'), css, function(err) {
          if (err) {
            return console.log(app.clr.red + 'autocompile write error! file' + app.clr.reset, file.replace('.less', '.css'), 'error:', err);
          }
        });
      });
    };
