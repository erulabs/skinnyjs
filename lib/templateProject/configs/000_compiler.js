        ...
        return function(err, rawCode) {
          ...
          if (error == null) {
            return app.fs.writeFile(file.replace('.coffee', '.js'), cs, function(err) {
              if (err) {
                return app.log(app.clr.red + 'autocompile write error! file' + app.clr.reset, file.replace('.coffee', '.js'), 'error:', err);
              }
            });
          }
        };
      ...
      return sass.render({
        ...
        error: (function(_this) {
          return function(error) {
            return app.log(app.clr.red + 'SCSS Compile error:' + app.clr.reset, error);
          };
        })(this)
      });
    };
    return app.compiler['.less'] = function(file) {
      ...
      return less.render(app.fs.readFileSync(file, 'utf8'), function(err, css) {
        return app.fs.writeFile(file.replace('.less', '.css'), css, function(err) {
          if (err) {
            return app.log(app.clr.red + 'autocompile write error! file' + app.clr.reset, file.replace('.less', '.css'), 'error:', err);
          }
        });
      });
    };
