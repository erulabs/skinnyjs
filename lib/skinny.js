  ...
  module.exports = Skinnyjs = (function() {
    ...
    Skinnyjs.prototype.initModule = function(type, opts) {
      ...
      if (typeof module !== 'function') {
        return console.log(this.clr.red + "WARNING:" + this.clr.reset, 'the', type.substr(0, type.length - 1), '"' + opts.name + '"', 'is malformed (not a function). It is being ignored');
      }
      try {
    ...
    Skinnyjs.prototype.error = function(error, opts) {
      ...
      return console.log(this.clr.red + 'Exception: ' + opts.error + this.clr.reset, 'in', (opts.details.name != null ? '"' + opts.details.name + '"' : opts), opts.type, "\n" + this.clr.cyan + "stack:" + this.clr.reset, error.stack);
    };
    ...
    Skinnyjs.prototype.init = function(cb) {
      ...
      try {
        this.httpd.listen(this.cfg.port, (function(_this) {
          return function() {
            return console.log('-->', _this.clr.green + 'Listening on port:' + _this.clr.reset, _this.cfg.port);
          };
        })(this));
      } catch (_error) {
        error = _error;
    ...
    Skinnyjs.prototype.fileChangeEvent = function(file) {
      ...
      if (this.fileMatch(file)) {
        ...
        if (this.initModule(file.split(this.path.sep).splice(-2)[0], {
          ...
          console.log('-->', this.clr.green + 'Reloading browser' + this.clr.reset, '-', file.replace(this.cfg.path, ''));
