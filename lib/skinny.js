...
(function() {
  ...
  module.exports = Skinnyjs = (function() {
    ...
    Skinnyjs.prototype.initModule = function(type, opts) {
      var error, isAssets, isClient, isntJavascript, module;
      if (opts.path == null) {
        return false;
      } else {
        this.path.normalize(opts.path);
      }
      isntJavascript = !!!opts.path.match(/\.js$/, isAssets = !!!opts.path.indexOf(this.path.sep + 'assets' + this.path.sep));
      isClient = !!!opts.path.indexOf(this.path.sep + 'client' + this.path.sep);
      if (isntJavascript || isAssets && isClient) {
        return true;
      }
    ...
    Skinnyjs.prototype.fileChangeEvent = function(file) {
      ...
      if (this.fileMatch(file)) {
        if (exists = this.fs.existsSync(file)) {
          ...
          console.log('compilter path:', this.compiler);
